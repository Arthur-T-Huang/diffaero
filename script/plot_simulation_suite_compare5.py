"""Compare v1 / v2 / v3 / v4 / v5 simulation suites side by side.

Reads results/simulation_suite/simulation_suite{,_v2,_v3,_v4,_v5}.csv and writes
simulation_suite_compare5.png alongside.
"""
...
OUT_PATH = ROOT / "simulation_suite_compare5.png"
...
VERSIONS = [
    ...
    ("v5", "simulation_suite_v5.csv", "#00838f", "#5d4037", "#33691e"),
]
...
def main():
    ...
    # delta annotation: v5 vs v3 (the relevant comparison after the v4 fix)
    for i in range(len(x)):
        d = successes["v5"][i] - successes["v3"][i]
        sign = "+" if d >= 0 else ""
    ...
    ax.set_title("v1 vs v2 vs v3 vs v4 vs v5: success rate per scenario (Δ above = v5 − v3)")
    ...
    ax2.set_title("Failure-mode shift across the five policies "
                  "(collision = solid bottom, timeout = stacked top)")
    ax2.legend(loc="upper left", framealpha=0.9, fontsize=6, ncol=5)
    ...
    ax3.set_title("Successful-episode flight speed across the five policies")
